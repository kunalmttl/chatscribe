...
function highlightCpp(code) {
  // First, escape HTML to avoid < and > messing up
  let s = esc(code);

  // Comments (must be first to avoid highlighting keywords inside)
  // We use placeholders to protect tokens from being double-processed
  const tokens = [];
  const reserve = (val, cls) => {
    const id = `___CHATSCRIBE_TOKEN_${tokens.length}___`;
    tokens.push(`<span class="token-${cls}">${val}</span>`);
    return id;
  };

  // 1. Comments
  s = s.replace(/\/\/.*|\/\*[\s\S]*?\*\//g, (m) => reserve(m, "com"));

  // 2. Strings
  s = s.replace(/"(?:\\.|[^\\"])*"|'(?:\\.|[^\\'])*'/g, (m) => reserve(m, "str"));

  // 3. Preprocessors
  s = s.replace(/(^|\n)(#\w+)/g, (_, p1, p2) => p1 + reserve(p2, "prep"));

  // 4. Keywords
  const kws = ["int", "void", "if", "else", "while", "for", "class", "public", "private", "protected", "return", "bool", "char", "float", "double", "unsigned", "signed", "long", "short", "struct", "union", "enum", "using", "namespace", "template", "typename", "virtual", "const", "static", "inline", "explicit", "export", "break", "continue", "switch", "case", "default", "try", "catch", "throw", "new", "delete", "this", "true", "false", "nullptr"];
  const kwRe = new RegExp(`\\b(${kws.join("|")})\\b`, "g");
  s = s.replace(kwRe, (m) => reserve(m, "kw"));

  // 5. Types / STL (Simplified)
  s = s.replace(/\b(std::\w+|string|vector|map|set|pair|list|unique_ptr|shared_ptr|uint\d+_t|int\d+_t|size_t)\b/g, (m) => reserve(m, "type"));

  // 6. Numbers
  s = s.replace(/\b(0x[\da-fA-F]+|0b[01]+|\d+(?:\.\d+)?(?:[eE][+-]?\d+)?(?:f|L|u|ll)?)\b/g, (m) => reserve(m, "num"));

  // Restore tokens
  for (let i = 0; i < tokens.length; i++) {
    s = s.replace(`___CHATSCRIBE_TOKEN_${i}___`, tokens[i]);
  }
...
export function mdToHtml(md) {
  ...
  while (i < lines.length) {
    ...
    if (fenceMatch) {
      ...
      const highlighted = (lang === "cpp" || lang === "c++") ? highlightCpp(content) : esc(content);
      out.push(`<pre${langAttr}><code${codeClass}>${highlighted}</code></pre>`);
      continue;
    }
